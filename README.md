...
- Vue 3 Viewer：Case 列表/搜索、Block 搜索、Case 文档、九种 Block Renderer、API 接口详情（输入输出 / 耗时来源 / HAR 单次请求瀑布）、时序图（UML 风格 lifeline + 有序消息箭头）、表格筛选、Artifact 类型筛选与内嵌预览、Gallery、Evidence。
...
支持的 Block：`markdown`、`facts`、`flow`、`table`、`timeline`、`evidence`、`gallery`、`api`、`sequence`。

`sequence` Block 承载按时间排序的交互（request → response → async-push）：`participants` 是 lifeline，`messages` 是有序箭头。消息 `kind` 区分 `sync`（实线）/ `async`（开口箭头）/ `stream`（虚线），并可选带 `status`、`durationMs` 与 `timingSource`（复用 `api` 的耗时来源枚举，让估算值始终有标注）。与 `flow`（拓扑图）分开，是因为 request→response→push 的顺序用图会丢失。
